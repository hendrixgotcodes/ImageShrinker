...
import * as imageManipulator from "./utils/imageManipulator";
import { handleOnOpenFolder, handleOnSelectFolder } from "./utils/ipcMainHandlers";
...
const createWindow = (): void => {
  ...
  const mainWindow = new BrowserWindow({
    ...
    icon:
      platform() === "win32"
        ? resolve("./src/assets/favicon.ico")
...
app.on("ready", () => {
  ipcMain.handle("dialog:selectFolder", handleOnSelectFolder);
  ipcMain.on("shell:openFolder", (_, folderPath:string) => {
    handleOnOpenFolder(folderPath);
  });
