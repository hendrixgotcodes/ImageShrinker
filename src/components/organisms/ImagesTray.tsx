...
import React, {
  MutableRefObject, useCallback, useContext, useEffect, useRef
} from "react";
...
export default function ImagesTray() {
  ...
  const imagePickerWrapperClass = classNames({
    ...
    "mr-14": images.length > 0
  });

  const handleImagePickerOnChange = useCallback((files:File[]) => {
    setImages([...images, ...files]);
  }, []);

  const handleOnImageDelete = useCallback((
    file: File,
